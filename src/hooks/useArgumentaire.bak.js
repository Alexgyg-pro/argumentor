...
export function useArgumentaire() {
  ...
  }, [needsRecalculation, calculateGlobalScore]);

  // + Bouton "Recalculer" pour forcer manuellement
  ...
  const onEditArgument = (id, newProperties) => {
    setArgumentTree((prevTree) => {
      ...
      if (nodeToEdit) {
        ...
        setNeedsRecalculation(true); // ← MARQUER RECALCUL
      }
      return newTree;
    });
    setIsDirty(true);
  ...
  // Exposer une fonction de recalcul manuel
